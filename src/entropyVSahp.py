...
def main():
    ...
    taskCount = np.array([])
    tasksAHPWeight = AHP_weights(["Task Size", "Task Deadline", "Output Size"], 3)
    ...
    for i in range(5):
        ...
        for iter in range(totalIter):
            ...
            [latencyEntropyTopsis, energyEntropyTopsis] = matchTaskandFogNode(
                entropyTopsisTaskOrdering, fogEntropyTopsisOrdering)
            [latencyEntropyMarcos, energyEntropyMarcos] = matchTaskandFogNode(
                entropyMarcosTaskOrdering, fogEntropyMarcosOrdering)
            [latencyEntropyMoora, energyEntropyMoora] = matchTaskandFogNode(
                entropyMooraTaskOrdering, fogEntropyMooraOrdering)
            [latencyEntropyVikor, energyEntropyVikor] = matchTaskandFogNode(
            ...
            [latencyAHPTopsis, energyAHPTopsis] = matchTaskandFogNode(
                AHPTopsisTaskOrdering, fogAHPTopsisOrdering)
            [latencyAHPMarcos, energyAHPMarcos] = matchTaskandFogNode(
                AHPMarcosTaskOrdering, fogAHPMarcosOrdering)
            [latencyAHPMoora, energyAHPMoora] = matchTaskandFogNode(
                AHPMooraTaskOrdering, fogAHPMooraOrdering)
            [latencyAHPVikor, energyAHPVikor] = matchTaskandFogNode(
        ...
        topsisAHPLatency = np.append(topsisAHPLatency, totLatencyAHPTopsis/totalIter)
        topsisAHPEnergy = np.append(topsisAHPEnergy, totEnergyAHPTopsis/totalIter)

        marcosAHPLatency = np.append(marcosAHPLatency, totLatencyAHPMarcos/totalIter)
        marcosAHPEnergy = np.append(marcosAHPEnergy, totEnergyAHPMarcos/totalIter)

        mooraAHPLatency = np.append(mooraAHPLatency, totLatencyAHPMoora/totalIter)
        mooraAHPEnergy = np.append(mooraAHPEnergy, totEnergyAHPMoora/totalIter)

        vikorAHPLatency = np.append(vikorAHPLatency, totLatencyAHPVikor/totalIter)
        vikorAHPEnergy = np.append(vikorAHPEnergy, totEnergyAHPVikor/totalIter)


        topsisEntropyLatency = np.append(topsisEntropyLatency, totLatencyEntropyTopsis/totalIter)
        topsisEntropyEnergy = np.append(topsisEntropyEnergy, totEnergyEntropyTopsis/totalIter)
        marcosEntropyLatency = np.append(marcosEntropyLatency, totLatencyEntropyMarcos/totalIter)
        marcosEntropyEnergy = np.append(marcosEntropyEnergy, totEnergyEntropyMarcos/totalIter)
        mooraEntropyLatency = np.append(mooraEntropyLatency, totLatencyEntropyMoora/totalIter)
        mooraEntropyEnergy = np.append(mooraEntropyEnergy, totEnergyEntropyMoora/totalIter)
        vikorEntropyLatency = np.append(vikorEntropyLatency, totLatencyEntropyVikor/totalIter)
        vikorEntropyEnergy = np.append(vikorEntropyEnergy, totEnergyEntropyVikor/totalIter)

        noOfTasks = noOfTasks+200

    generatePlot2(taskCount, topsisEntropyEnergy, topsisAHPEnergy, marcosEntropyEnergy, marcosAHPEnergy, mooraEntropyEnergy, mooraAHPEnergy, vikorEntropyEnergy, vikorAHPEnergy,"Task Count", "Energy", "with Entropy TOPSIS", "with AHP TOPSIS", "with Entropy MARCOS", "with AHP MARCOS", "with Entropy MOORA", "with AHP MOORA", "With Entropy VIKOR", "With AHP VIKOR")

    generatePlot2(taskCount, topsisEntropyLatency, topsisAHPLatency, marcosEntropyLatency, marcosAHPLatency, mooraEntropyLatency, mooraAHPLatency, vikorEntropyLatency, vikorAHPLatency,"Task Count", "Latency", "with Entropy TOPSIS", "with AHP TOPSIS", "with Entropy MARCOS", "with AHP MARCOS", "with Entropy MOORA", "with AHP MOORA", "With Entropy VIKOR", "With AHP VIKOR")
